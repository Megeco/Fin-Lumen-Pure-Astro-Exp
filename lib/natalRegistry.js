...
import v35CandidateCharts from "./v35CandidateCharts.js";
...
function normalizeEntry(symbol, entry = {}) {
  ...
  const v35Candidates = v35CandidateCharts[symbol] || {};
  const baseCharts = Array.isArray(entry.charts) && entry.charts.length ? entry.charts : [legacyChart(entry)].filter(Boolean);
  const charts = mergeCharts(baseCharts, v35Candidates.charts, override.charts, research.charts);
