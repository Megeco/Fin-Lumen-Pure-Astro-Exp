...
function receptorFitCapitalGate(transitReceptorFit, capitalHorizon) {
  ...
  if (blockFresh) {
    next.tactical = {
      ...(next.tactical || {}),
      verdict: "NO FRESH ENTRY — PROTECT / OBSERVE",
      displayVerdict: "Fresh allocation not favoured; protect/observe.",
      note: `${label}: ${reading || "Transit receptor fit is weak or pressure-led."}`
    };
    next.strategic = {
      ...(next.strategic || {}),
      verdict: "AVOID FRESH STRATEGIC CAPITAL",
      displayVerdict: "Fresh strategic allocation not favoured.",
      note: "Macro weather does not convert into investable company-specific expression through sector/receptor/pressure checks."
    };
    next.passiveLongTerm = {
      ...(next.passiveLongTerm || {}),
      verdict: "NO — NOT PASSIVE LONG-TERM",
      displayVerdict: "Not suitable for passive long-term capital at this window.",
      note: "Company-specific transit expression is pressure/non-responsive until a new receptor window appears."
    };
  } else if (muted) {
    next.tactical = {
      ...(next.tactical || {}),
