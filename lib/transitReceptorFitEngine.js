...
import { getHistoricalRegistry } from "./historical/historicalResponseStore.js";
...
export function evaluateTransitReceptorFit({ company = {}, natal = {}, transits = {}, replay = {}, macro = null, fundamentals = null } = {}) {
  ...
  const historicalEcho = scoreHistoricalEcho(company, replay, transit, sectorProfile);
  ...
  const rawScore =
    ...
    0.15 * sectorThemeFit.score +
    0.20 * natalReceptorStrength.score +
    0.15 * supportNetworkQuality.score +
    0.15 * fundamentalTransmission.score +
    0.10 * historicalEcho.score +
  ...
  const confidenceScore = clamp(Math.round(
    natalReliability * 0.35 +
    historicalEcho.score * 0.25 +
    natalReceptorStrength.score * 0.20 +
    supportNetworkQuality.score * 0.10 +
  ...
  return {
    ...
    version: "v32-trm-1",
    principle: "Macro transit creates weather; sector fit creates eligibility; natal receptor creates expression; replay creates confidence; fundamentals create investability.",
    transit: {
    ...
    scores: {
      ...
      historicalEcho: Math.round(historicalEcho.score),
