...
function findNextMacroAspect(pair, aspect, startDate, scanDays = 120) {
  ...
  return {
    ...
    source: "active ephemeris provider aspect scan + local refinement",
    eventPrecision: "runtime-refined",
    precisionNote: "Timestamp is refined at runtime from the active ephemeris provider.",
...
function buildUpcomingAspectCandidates(today) {
  const staticCandidates = getStaticSwissUpcomingEvents(today, "macroAspects", 80, 180);

  if (staticCandidates.length) {
    return staticCandidates
      .map(normalizeStaticEvent)
      .filter(Boolean)
      .filter(item => Number(item.daysRemaining ?? item.daysTill ?? 999) <= 180)
...
function normalizeStaticEvent(item) {
  ...
  return {
    ...
    eventPrecision: item.eventPrecision || (String(item.source || "").includes("interpolated") ? "interpolated-daily-swiss" : "exact-swiss-event"),
    precisionNote: item.precisionNote || (String(item.source || "").includes("interpolated")
      ? "Timestamp is interpolated/refined from the static Swiss daily-position table; later pass can replace with full binary-searched Swiss timestamp."
      : "Timestamp comes from the static Swiss event table."
    )
  };
}
...
function hardenEclipseEvent(item) {
  ...
  return {
    ...
    eventPrecision: item.eventPrecision || "exact-swiss-event",
    precisionNote: item.precisionNote || "Timestamp comes from the static Swiss event table."
  };
}
...
function buildIngressCandidates(today) {
  const staticIngresses = getStaticSwissUpcomingEvents(today, "ingresses", 12, 1095)
    .map(normalizeStaticEvent);

  if (staticIngresses.length) {
    return staticIngresses;
  }
...
function buildRetrogradeCandidates(today) {
  const staticStations = getStaticSwissUpcomingEvents(today, "stations", 16, 730)
    .map(normalizeStaticEvent);
...
function buildUpcomingEclipseCandidates(today) {
  const staticEclipses = getStaticSwissUpcomingEvents(today, "eclipses", 6, 365)
    .map(normalizeStaticEvent);

  if (staticEclipses.length) {
    return staticEclipses;
  }
